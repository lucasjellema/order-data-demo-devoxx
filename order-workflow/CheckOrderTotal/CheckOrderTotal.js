...
var APP_NAME = "CheckOrderTotal"

console.log("Running CheckOrderTotal version " + APP_VERSION);
    ...
    for (i = 0; i < event.actions.length; i++) {
      ...
      if ("CheckOrderTotal" == action.type) {
        ...
        if ("new" == action.status && conditionsSatisfied(action, event.actions)) {
          ...
          var outcome = checkOrderTotal(event.payload);
          ...
          // add audit line
          event.audit.push(
            { "when": new Date().getTime(), "who": "CheckOrderTotal", "what": "update", "comment": "Order Total Check Complete" }
          );

          acted = true;
          localLoggerAPI.log("Check Order Total (outcome:" + JSON.stringify(outcome) + ")"
